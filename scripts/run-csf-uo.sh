...
opath="${output_dir}/K${feature}/${typename}-${fixed_size}"
...
do
    ...
    OMP_NUM_THREADS=${threads} OMP_PROC_BIND=true OMP_PLACES="cores(${cores})" ${executable} --m ${matrix_path} --feature ${feature} --threads ${threads} --numruns ${nruns} --layers ${layers} --residue ${residue} --greedy ${greedy} --kernel ${kernel} --fixed-nnzs ${fixed_nnzs} --type "CSF-UO" 2>$efile >$tfile
